...
if (class_exists('Meme_AATeam_Product_Updater') != true) {
    class Meme_AATeam_Product_Updater
    {
    	/*
        ...
        */
		...
		protected $key_alias = 'kingdom_register_key';
		 ...
		 * @var string
		 */
		public $title = 'Premium Meme Generator / Maker Wordpress Plugin';
        ...
        */
        ...
        { 
			...
			//$this->product_path = 'js-composer-extend-kingdom/vc-extend.php';
		...
		public function custom_update_style() {
			wp_enqueue_style('kd-updater-style', plugins_url( 'updater.css', __FILE__) );
		}
		...
		protected function getUrl() {
			...
			if( $this->product_slug == 'premium-meme-generator' ) {
		...
		public function preUpgradeFilter( $reply, $package, $updater ) {
			...
			if ( ! $res ) {
				return new WP_Error( 'no_credentials', __( "Error! Can't connect to filesystem", 'premium-meme-generator' ) );
			}
			
			$updater->strings['downloading_package_url'] = __( 'Getting download link...', 'premium-meme-generator' );
			$updater->skin->feedback( 'downloading_package_url' );
	
			$response = $this->getDownloadUrl();
			  
			if ( ! $response ) {
				return new WP_Error( 'no_credentials', __( 'Download link could not be retrieved', 'premium-meme-generator' ) );
			}
			...
			$updater->strings['downloading_package'] = __( 'Downloading package...', 'kingdom-vc' );
		...
		public function check_update( $transient ) {
			...
			if ( version_compare( $this->current_version, $remote_version, '<' ) ) {
				...
				$obj->name = 'Premium Meme Generator / Maker Wordpress Plugin';
